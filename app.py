...
def carregar_dados(arquivo) -> pd.DataFrame:
    ...
    # Converte colunas numéricas — aceita vírgula como separador decimal
...
# Atalhos
C = COLS
df_valido = df[df[C["vlr_frete"]] > 0].copy()
...
agg_uf["rs_por_kg"]     = agg_uf.apply(
    lambda r: r["total_vendas"] / r["total_peso"] if r["total_peso"] > 0 else 0, axis=1
)
...
display_uf.columns = [
    ...
    "Peso Total (Kg)", "Qtd Pedidos", "R$/Kg Médio", "Frete/Venda (%)"
]
...
display_uf["R$/Kg Médio"]      = display_uf["R$/Kg Médio"].apply(lambda v: f"R$ {v:.2f}")
...
# ── Filtro rápido de Clientes e Transportadoras por UF ───────────────────────
...
with col_cli:
    ...
    clientes_uf = (
        df_uf_p1.groupby(C["cliente"], as_index=False)[C["vlr_pedido"]]
        .sum()
        .sort_values(C["vlr_pedido"], ascending=False)
    )
    clientes_uf.columns = ["Cliente", "Total Vendas (R$)"]
    clientes_uf["Total Vendas (R$)"] = clientes_uf["Total Vendas (R$)"].apply(formata_moeda)
    st.dataframe(clientes_uf, use_container_width=True, hide_index=True)

with col_transp:
    st.markdown(f"**🚛 Transportadoras em {uf_selecionada_p1}**")
    transp_uf = (
        df_uf_p1.groupby(C["transportadora"], as_index=False)[C["vlr_frete"]]
        .sum()
        .sort_values(C["vlr_frete"], ascending=False)
    )
    transp_uf.columns = ["Transportadora", "Total Frete (R$)"]
    transp_uf["Total Frete (R$)"] = transp_uf["Total Frete (R$)"].apply(formata_moeda)
    st.dataframe(transp_uf, use_container_width=True, hide_index=True)
...
with st.expander("📄 Ver tabela completa por cliente"):
    ...
    tbl_cli.columns = ["Cliente", "Total Venda (R$)", "Total Frete (R$)", "Frete/Venda (%)"]
...
if not rotas:
    ...
    df_rota = df_rota[df_rota[C["peso"]] > 0]  # só linhas com peso válido
    ...
    if df_rota.empty:
        ...
        # ── Dispersão individual de fretes na rota ────────────────────────────
        with st.expander("📊 Ver dispersão de fretes individuais nesta rota"):
            if df_rota[C["peso"]].sum() > 0:
